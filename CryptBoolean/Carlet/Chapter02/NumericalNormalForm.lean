...
/-!
# Carlet Chapter 2 numerical normal form

Existence and uniqueness of the multilinear real representation of a
pseudo-Boolean function. The proof uses the same Boolean-lattice zeta
injectivity as ANF and finite-dimensional linear algebra from Mathlib.
-/
...
abbrev PseudoBooleanFunction (n : ℕ) := FABL.F₂Cube n → ℝ
...
def numericalMonomial (S : Finset (Fin n)) (x : FABL.F₂Cube n) : ℝ :=
...
theorem numericalMonomial_f₂CubeOfFinset (S U : Finset (Fin n)) :
    numericalMonomial S (FABL.f₂CubeOfFinset U) = if S ⊆ U then 1 else 0 := by
  classical
  ...
  · rw [if_pos h]
    ...
    intro i hi
    simp [FABL.f₂CubeOfFinset_apply, h hi]
  · rw [if_neg h]
    obtain ⟨i, hiS, hiU⟩ := Finset.not_subset.mp h
    apply Finset.prod_eq_zero hiS
    simp [FABL.f₂CubeOfFinset_apply, hiU]
...
theorem numericalEval_f₂CubeOfFinset (c : NumericalCoefficients n)
    (U : Finset (Fin n)) :
    numericalEval c (FABL.f₂CubeOfFinset U) = ∑ S ∈ U.powerset, c S := by
  classical
  rw [numericalEval]
  calc
    ∑ S, c S * numericalMonomial S (FABL.f₂CubeOfFinset U) =
...
theorem numericalEval_injective :
  ...
  exact congrFun h (FABL.f₂CubeOfFinset U)
...
theorem finrank_numericalCoefficients_eq_pseudoBooleanFunction :
  ...
  rw [Module.finrank_fintype_fun_eq_card, Module.finrank_fintype_fun_eq_card,
    Fintype.card_finset, Fintype.card_fin, card_f₂Cube]
...
theorem numericalCoeff_eq_value_sub_lower
  ...
  have heval := congrFun (numericalEval_numericalCoeff φ) (FABL.f₂CubeOfFinset S)
...
noncomputable def numericalMobiusCoeff
  ...
  fun S ↦ ∑ T ∈ S.powerset,
    (-1 : ℝ) ^ (S.card - T.card) * φ (FABL.f₂CubeOfFinset T)
...
theorem numericalEval_numericalMobiusCoeff_f₂CubeOfFinset
    ...
    numericalEval (numericalMobiusCoeff φ) (FABL.f₂CubeOfFinset U) =
      φ (FABL.f₂CubeOfFinset U) := by
  classical
  ...
  have step1 : ∀ S ∈ U.powerset,
      (∑ T ∈ S.powerset,
        ...
        ∑ T ∈ U.powerset, if T ⊆ S then
          (-1 : ℝ) ^ (S.card - T.card) * φ (FABL.f₂CubeOfFinset T) else 0 := by
    intro S hS
  ...
  have step2 : ∀ T ∈ U.powerset,
      (∑ S ∈ U.powerset, if T ⊆ S then
          (-1 : ℝ) ^ (S.card - T.card) * φ (FABL.f₂CubeOfFinset T) else 0) =
        if T = U then φ (FABL.f₂CubeOfFinset U) else 0 := by
    intro T hT
    ...
    calc
      ∑ S ∈ Finset.Icc T U,
          (-1 : ℝ) ^ (S.card - T.card) * φ (FABL.f₂CubeOfFinset T) =
          (∑ S ∈ Finset.Icc T U, (-1 : ℝ) ^ (S.card - T.card)) *
            φ (FABL.f₂CubeOfFinset T) := by rw [Finset.sum_mul]
      _ = (if T = U then 1 else 0) * φ (FABL.f₂CubeOfFinset T) := by
        rw [sum_Icc_neg_one_pow_card_sub T U hT]
      _ = if T = U then φ (FABL.f₂CubeOfFinset U) else 0 := by
...
theorem numericalCoeff_eq_mobius_sum
    ...
    numericalCoeff φ S = ∑ T ∈ S.powerset,
      (-1 : ℝ) ^ (S.card - T.card) * φ (FABL.f₂CubeOfFinset T) := by
