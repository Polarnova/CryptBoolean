...
| `carlet-4-six-variable-covering-coset-coordinate` | Lemma in Hou's proof of the exact dimension-seven covering radius: a six-variable coset at nonlinearity `28` has a minimum affine representative whose error contains any prescribed coordinate. | The associated declaration proves the statement for every Boolean function and coordinate; the degree-four restriction used in Hou's surrounding case analysis is unnecessary. Flat Walsh spectrum and inversion give the result. |
| `carlet-4-six-variable-degree-four-coset-coordinate` | Lemma in Hou's proof of the exact dimension-seven covering radius: a degree-at-most-four six-variable coset at nonlinearity `26` has a minimum affine representative whose error contains any prescribed coordinate. | Reed--Muller duality gives affine-coset weights modulo four; Relation (35), Parseval, and Walsh inversion force the coordinate-covering leader. |
...
**Proof of Proposition 13.** Carlet refers the omitted proof to reference [72]. The argument follows
that source's two steps: differentiating a closest order-`r` Reed--Muller
...
autocorrelation, and square-root steps used by this composition, with both bounds proved from these
ingredients.

## Reviewed formalized surface

The 115 formalized statement nodes are split by mathematical result. Implementation module
boundaries do not determine this split. The fidelity column records how the compiled declarations
meet the displayed source mathematics.

| Family | Formalized Blueprint items | Fidelity | Lean declarations |
|---|---|---|---:|
| Boolean foundations and raw Walsh transform | `carlet-2-def-boolean-function`, `carlet-2-def-support-weight`, `carlet-2-def-walsh-transform`, `carlet-2-walsh-normalization`, `carlet-2-balanced-zero-walsh` | Exact definitions and results, including the Walsh normalization identity | 19 |
...
| Algebraic degree, distance, and affine functions | `carlet-2-def-algebraic-degree`, `carlet-2-support-degree-addition`, `carlet-2-def-hamming-distance`, `carlet-2-relative-hamming-normalization`, `carlet-2-def-affine-functions` | Exact source items, the relative-distance normalization, and the derived addition law | 18 |
...
| Spectral-support bounds | `carlet-2-spectral-support-bounds` | Exact with explicit zero-function conventions and the raw/normalized identity | 24 |
